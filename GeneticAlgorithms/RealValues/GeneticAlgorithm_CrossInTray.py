...
class NQueens:
    def __init__(self, dimension, gene = None):
        ...
        if gene is None:
            ...
            for i in range(dimension):
                self.gene.append(random.uniform(-10, 10))
        else:
            self.gene = gene
    ...
    def getChildren(self, partner):
        ...
        while j < self.dimension:
            ...
            if value < -10:
                self.children[0].gene.append(-10)
            elif value > 10:
                self.children[0].gene.append(10)
            else:
            ...
            value = self.gene[j] + (random.gauss(0, 0.25) * x)
            if value < -10:
                self.children[1].gene.append(-10)
            elif value > 10:
                self.children[1].gene.append(10)
            else:
...
class GameOfQueens:
    ...
    def fitness(self, pop):
        k = 0
        while k < len(pop):
            funcMath = math.pow(math.fabs(
                math.sin(pop[k].gene[0]) * 
                math.sin(pop[k].gene[1]) * 
                math.exp(
                        math.fabs(100 - ( math.sqrt(math.pow(pop[k].gene[0], 2) + math.pow(pop[k].gene[1], 2)) / math.pi) ))) + 1, 0.1 )

            fitness = -0.0001 * funcMath

            pop[k].realFitness = fitness
            pop[k].falseFitness = 1 / fitness + 2.06262
